...
const _createOffer = async (props, wallet, session, dappKeyPair, sharedSecret) => {
    ...
    const admin = new PublicKey(poolData.admin);
    ...
    const adminAccount = await getTokenWallet(admin, mint);
    if((await connection.getAccountInfo(adminAccount)) == null){
        transaction.add(createAssociatedTokenAccountInstruction(adminAccount, wallet, admin, mint))
    }
